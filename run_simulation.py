...
def parse_orca_log(log_file):
    ...
    for agent_log in log_section.findall("agent"):
        ...
        start_x_grid, start_y_grid = float(agent_def.get("start.xr")), float(
            agent_def.get("start.yr")
        )
        goal_x_grid, goal_y_grid = float(agent_def.get("goal.xr")), float(
            agent_def.get("goal.yr")
        )
        ...
        if path is not None:
            ...
            for i in range(len(steps)):
                ...
                if i < len(steps) - 1:
                    next_step = steps[i + 1]
                    next_pos_grid = [
                        float(next_step.get("xr")),
                        float(next_step.get("yr")),
                    ]
                    next_pos_world = list(
                        grid_to_world(next_pos_grid[0], next_pos_grid[1])
                    )
                    # Calculate velocity as (next_pos - pos) / time_step
                    vel = [
                        (next_pos_world[0] - pos_world[0]) / time_step,
                        (next_pos_world[1] - pos_world[1]) / time_step,
                    ]
                else:
...
def generate_orca_csvs(log_file, output_dir):
    ...
    for agent in agents_data:
        ...
        nominal_x, nominal_y = calculate_nominal_path(
            agent["start_pos"], agent["goal_pos"], num_steps
        )
...
def evaluate_velocities(velocity_csv, verbose=True):
    ...
    for robot_id in robot_ids:
        ...
        data[f"robot_{robot_id}_resultant"] = np.sqrt(
            data[vx_col] ** 2 + data[vy_col] ** 2
        )
...
def generate_animation(
    agents_data, output_dir, map_size=(64, 64), config_file=None, num_robots=None
):
    ...
    dyn_scatter = ax.scatter(
        [], [], c=[], s=200, edgecolors="black", linewidths=1, label="Agent"
    )
    obs_scatter = ax.scatter(
        [], [], c="gray", s=200, edgecolors="black", linewidths=1, label="Obstacle"
    )
    ...
    )  # Same as IMPC-DR (r_min/2.0 where r_min=0.3)
    ...
    try:
        ...
        anim.save(
            animations_dir / animation_filename, writer="pillow", fps=5
        )  # Set FPS to 5
...
def run_social_orca_standardized(env_type, verbose=False):
    ...
    if env_type == "doorway":
        ...
        print(
            "- The doorway has a vertical wall at x=0 with a gap between y=-2 and y=2"
        )
    ...
    standard_positions = StandardizedEnvironment.get_standard_agent_positions(
        env_type, num_moving_robots
    )
    ...
    for i in range(num_moving_robots):
        ...
        # Get start position
        start_x = get_input(
            f"Start X position (default: {defaults['start_x']})",
            defaults["start_x"],
            float,
        )
        start_y = get_input(
            f"Start Y position (default: {defaults['start_y']})",
            defaults["start_y"],
            float,
        )
        ...
        # Get goal position
        goal_x = get_input(
            f"Goal X position (default: {defaults['goal_x']})",
            defaults["goal_x"],
            float,
        )
        goal_y = get_input(
            f"Goal Y position (default: {defaults['goal_y']})",
            defaults["goal_y"],
            float,
        )
        ...
        print(
            f"Agent {i+1} configured: Start=({start_x}, {start_y}), Goal=({goal_x}, {goal_y})"
        )
...
def get_standardized_orca_config(env_type):
    ...
    if env_type == "doorway":
        ...
        config_file = (
            orca_dir / "configs" / "config_intersection_2_robots_standardized.xml"
        )
    else:
        # Fallback to doorway
        config_file = orca_dir / "configs" / "config_doorway_2_robots_standardized.xml"
...
def build_social_orca():
    ...
    try:
        ...
        result = subprocess.run(
            ["make", "build/single_test"], capture_output=True, text=True
        )
...
def run_social_orca(config_file, num_robots, verbose=False):
    ...
    try:
        ...
        animation_path = generate_animation(
            agents_data, output_dir, config_file=config_path, num_robots=num_robots
        )
        ...
        for i, traj_file in enumerate(trajectory_files):
            ...
            hausdorff_dist = directed_hausdorff(actual_trajectory, nominal_trajectory)[
                0
            ]
        ...
        if agent_completion_data:
            agents_reached_goals = [
                data for data in agent_completion_data if data["reached_goal"]
            ]
            agents_failed = [
                data for data in agent_completion_data if not data["reached_goal"]
            ]

            if verbose:
                print(
                    f"Agents that reached goals: {len(agents_reached_goals)}/{len(agent_completion_data)}"
                )

            if agents_reached_goals:
                # Get the completion time of the last agent to reach its goal
                goal_completion_times = [
                    data["completion_time"] for data in agents_reached_goals
                ]
                ...
                if len(agents_reached_goals) == len(agent_completion_data):
                    ...
                    if verbose:
                        ...
                        print(
                            f"Individual completion times: {[f'{t:.2f}s' for t in goal_completion_times]}"
                        )
                else:
                    # Some agents didn't reach goals - use total simulation time for fairness
                    all_completion_times = [
                        data["completion_time"] for data in agent_completion_data
                    ]
                    makespan = (
                        max(all_completion_times)
                        if all_completion_times
                        else (total_makespan or latest_goal_completion)
                    )
                    if verbose:
                        print(
                            f"Not all agents reached goals. Using total simulation time: {makespan:.2f}s"
                        )
                        print(
                            f"Successful agents completed at: {[f'{t:.2f}s' for t in goal_completion_times]}"
                        )
                        if agents_failed:
                            failed_times = [
                                data["completion_time"] for data in agents_failed
                            ]
                            print(
                                f"Failed agents stopped at: {[f'{t:.2f}s' for t in failed_times]}"
                            )
            else:
                    ...
                    if len(obstacles) == 8:
                        ...
                        if len(horizontal_walls) >= 2:
                            horizontal_walls.sort(
                                key=lambda x: x[0]
                            )  # Sort by y position in wall tuples
        ...
        if makespan > 0 and gap_width > 0:
            ...
            if agent_completion_data:
                ...
                if successful_agents == num_robots:
                    ...
                    flow_rate_type = (
                        f"Only {successful_agents}/{num_robots} agents reached goals"
                    )
                else:
...
def run_social_impc_dr(env_type="doorway", verbose=False):
    """Run Social-IMPC-DR through its standard command-line entry point."""
    ...
    try:
        ...
        script_path = impc_dir / "standard_cli.py"
        ...
        try:
            ...
            # Clear any cached module to avoid reusing state from an earlier run.
            if "standard_cli" in sys.modules:
                del sys.modules["standard_cli"]

            # Try to run the command-line module directly.
            try:
                import standard_cli

                print("✓ Successfully imported standard_cli")

                # Call the entry point with environment type and verbose mode.
                ...
                sys.argv = [
                    "standard_cli.py",
                    env_type,
                    "--verbose" if verbose else "--clean",
                ]

                try:
                    # Call the main function if it exists, otherwise run the script
                    if hasattr(standard_cli, "main"):
                        result = standard_cli.main()
                    else:
                    ...
                    ):  # Some scripts may not return a value
                        print("✓ IMPC-DR simulation completed successfully!")

                        # Look for generated trajectory files
                        path_deviation_files = list(
                            impc_dir.glob("path_deviation_robot_*.csv")
                        )
                        if path_deviation_files:
                            print(
                                f"✓ Found {len(path_deviation_files)} trajectory files"
                            )
                            ...
                            trajectory_results = evaluate_impc_trajectories(
                                impc_dir,
                                env_type,
                                path_deviation_files,
                                verbose=verbose,
                            )
                            velocity_metrics = evaluate_impc_velocities(
                                impc_dir, verbose=verbose
                            )
                ...
                result = subprocess.run(
                    [get_venv_python(), "standard_cli.py", env_type, verbose_arg],
                    cwd=impc_dir,
                    capture_output=True,
                    text=True,
                )
                ...
                if result.returncode == 0:
                    ...
                    if path_deviation_files:
                        ...
                        # Evaluate trajectories and velocities
                        trajectory_results = evaluate_impc_trajectories(
                            impc_dir, env_type, path_deviation_files, verbose=verbose
                        )
                        velocity_metrics = evaluate_impc_velocities(
                            impc_dir, verbose=verbose
                        )
...
def evaluate_impc_trajectories(impc_dir, env_type, path_deviation_files, verbose=True):
    ...
    # Use the standard CLI's default step size.
    step_size = 0.1
    ...
    if os.path.exists(completion_step_file):
        try:
            ...
            if verbose:
                print(
                    f"Using actual completion time: {actual_completion_steps} steps (goals reached)"
                )
        except:
            if verbose:
                print(
                    f"Could not read completion step file, using max steps: {max_steps}"
                )
    else:
        if verbose:
    ...
    if os.path.exists(ttg_file):
        ...
        success_rate = (
            (successful_count / total_robots) * 100.0 if total_robots > 0 else 0.0
        )
        ...
        if len(successful_robots) > 0:
            ...
            if verbose:
                ...
                print("Makespan Ratios (MR_i = TTG_i / TTG_fastest):")
                print(
                    f"Note: Only robots that reached their goals are included in makespan ratio calculation"
                )
            ...
            for idx, row in ttg_df.iterrows():
                ...
                if reached_goal:
                    ...
                    if verbose:
                        print(
                            f"Robot {robot_id}: TTG = {ttg}, MR = {mr:.4f} ✓ (reached goal)"
                        )
                else:
                    # Robot didn't reach goal
                    ttg_metrics[robot_id] = {
                        "ttg": ttg,
                        "mr": float("inf"),
                        "reached_goal": False,
                    }
                    if verbose:
                        print(
                            f"Robot {robot_id}: TTG = {ttg}, MR = N/A ✗ (did not reach goal)"
                        )
            if verbose:
                print("*" * 65)
        else:
...
def display_clean_impc_metrics(
    trajectory_metrics,
    velocity_metrics,
    ttg_metrics,
    flow_rate,
    ...
    successful_count = sum(
        1
        for robot_id in ttg_metrics
        if ttg_metrics[robot_id].get("reached_goal", False)
    )

    print(
        f"Environment: {environment}  Success Rate: {success_rate:.1f}% ({successful_count}/{num_agents})  Makespan: {makespan:.2f}s  Flow Rate: {flow_rate:.4f}"
    )
    print()
    ...
    robot_ids = sorted(
        set(
            list(trajectory_metrics.keys())
            + list(velocity_metrics.keys())
            + list(ttg_metrics.keys())
        )
    )
    ...
    for robot_id in robot_ids:
        ...
        print(
            f"Robot {robot_id}   {ttg:<3}  {mr_str:<6} {avg_delta_v:<6.3f}  {path_dev:<8.3f}  {hausdorff:<8.3f}"
        )


def display_clean_orca_metrics(
    trajectory_metrics,
    velocity_metrics,
    ttg_metrics,
    flow_rate,
    ...
    for robot_id in robot_ids:
        ...
        mr = (
            ttg / fastest_ttg
            if ttg != float("inf") and fastest_ttg > 0
            else float("inf")
        )
        ...
        print(
            f"Robot {robot_id}   {ttg_str:<3}  {mr_str:<6} {avg_delta_v:<6.3f}  {path_dev:<8.3f}  {hausdorff:<8.3f}"
        )
...
def generate_config(env_type, num_robots, robot_positions):
    ...
    for i in range(num_robots):
        # Convert coordinates
        start_x_grid, start_y_grid = world_to_grid(
            robot_positions[i]["start_x"], robot_positions[i]["start_y"]
        )
        goal_x_grid, goal_y_grid = world_to_grid(
            robot_positions[i]["goal_x"], robot_positions[i]["goal_y"]
        )
    ...
    if env_type == "hallway":
        ...
        ET.SubElement(
            obstacle1, "vertex", {"xr": str(x_left_grid), "yr": str(y_bottom_wall_grid)}
        )
        ET.SubElement(
            obstacle1,
            "vertex",
            {"xr": str(x_right_grid), "yr": str(y_bottom_wall_grid)},
        )
        ET.SubElement(
            obstacle1,
            "vertex",
            {"xr": str(x_right_grid), "yr": str(y_bottom_wall_grid + 1)},
        )
        ET.SubElement(
            obstacle1,
            "vertex",
            {"xr": str(x_left_grid), "yr": str(y_bottom_wall_grid + 1)},
        )
        ...
        ET.SubElement(
            obstacle2, "vertex", {"xr": str(x_left_grid), "yr": str(y_top_wall_grid)}
        )
        ET.SubElement(
            obstacle2, "vertex", {"xr": str(x_right_grid), "yr": str(y_top_wall_grid)}
        )
        ET.SubElement(
            obstacle2,
            "vertex",
            {"xr": str(x_right_grid), "yr": str(y_top_wall_grid + 1)},
        )
        ET.SubElement(
            obstacle2,
            "vertex",
            {"xr": str(x_left_grid), "yr": str(y_top_wall_grid + 1)},
        )
        ...
        obstacle1 = ET.SubElement(obstacles1, "obstacle")
        ET.SubElement(
            obstacle1, "vertex", {"xr": str(x_wall_grid), "yr": str(y_bottom_grid)}
        )
        ET.SubElement(
            obstacle1, "vertex", {"xr": str(x_wall_grid), "yr": str(y_gap_bottom_grid)}
        )
        ET.SubElement(
            obstacle1,
            "vertex",
            {"xr": str(x_wall_grid + 1), "yr": str(y_gap_bottom_grid)},
        )
        ET.SubElement(
            obstacle1, "vertex", {"xr": str(x_wall_grid + 1), "yr": str(y_bottom_grid)}
        )
        ...
        obstacle2 = ET.SubElement(obstacles2, "obstacle")
        ET.SubElement(
            obstacle2, "vertex", {"xr": str(x_wall_grid), "yr": str(y_gap_top_grid)}
        )
        ET.SubElement(
            obstacle2, "vertex", {"xr": str(x_wall_grid), "yr": str(y_top_grid)}
        )
        ET.SubElement(
            obstacle2, "vertex", {"xr": str(x_wall_grid + 1), "yr": str(y_top_grid)}
        )
        ET.SubElement(
            obstacle2, "vertex", {"xr": str(x_wall_grid + 1), "yr": str(y_gap_top_grid)}
        )
        ...
        ET.SubElement(
            obstacle3, "vertex", {"xr": str(x_left_wall_grid), "yr": str(y_bottom_grid)}
        )
        ET.SubElement(
            obstacle3, "vertex", {"xr": str(x_left_wall_grid), "yr": str(y_top_grid)}
        )
        ET.SubElement(
            obstacle3,
            "vertex",
            {"xr": str(x_left_wall_grid + 1), "yr": str(y_top_grid)},
        )
        ET.SubElement(
            obstacle3,
            "vertex",
            {"xr": str(x_left_wall_grid + 1), "yr": str(y_bottom_grid)},
        )
        ...
        ET.SubElement(
            obstacle4,
            "vertex",
            {"xr": str(x_right_wall_grid), "yr": str(y_bottom_grid)},
        )
        ET.SubElement(
            obstacle4, "vertex", {"xr": str(x_right_wall_grid), "yr": str(y_top_grid)}
        )
        ET.SubElement(
            obstacle4,
            "vertex",
            {"xr": str(x_right_wall_grid + 1), "yr": str(y_top_grid)},
        )
        ET.SubElement(
            obstacle4,
            "vertex",
            {"xr": str(x_right_wall_grid + 1), "yr": str(y_bottom_grid)},
        )
        ...
        try:
            ...
            try:
                ...
                if result is not None:
                    ...
                    animations_dir = (
                        Path(__file__).parent / "logs" / "Social-CADRL" / "animations"
                    )
