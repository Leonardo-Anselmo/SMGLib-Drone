...
def calculate_nominal_path(
    start_pos: Tuple[float, float], goal_pos: Tuple[float, float], num_steps: int
...
def calculate_path_deviation(
    ...
    for i in range(min_len):
        deviation = np.sqrt(
            (actual_x[i] - nominal_x[i]) ** 2 + (actual_y[i] - nominal_y[i]) ** 2
        )
...
def create_animation(
    ...
    agent_scatter = ax.scatter(
        [], [], c=[], s=200, edgecolors="black", linewidths=1, label="Agents"
    )
                ...
                if len(vertices) >= 3:
                    ...
                    if (
                        ...
                        rect = patches.Rectangle(
                            (
                                min(float(x1.text), float(x2.text)),
                                min(float(y1.text), float(y2.text)),
                            ),
                            width,
                            height,
                            linewidth=0,
    ...
    anim = FuncAnimation(
        ...
        animate,
        frames=max_frames,
        interval=int(max(time_step, 0.05) * 1000 * 3),
        blit=True,
    )
    ...
    try:
        ...
        anim.save(
            gif_path,
            writer="pillow",
            fps=round(1000 / max(1, int(max(time_step, 0.05) * 1000 * 3))),
        )
...
def print_simulation_results(
    ...
    successful_agents = sum(
        1 for agent in completion_data if agent.get("reached_goal", False)
    )
    print(f"Agents that reached goals: {successful_agents}/{len(completion_data)}")

    # Individual agent results
    completion_times = [
        agent.get("completion_time", float("inf")) for agent in completion_data
    ]
    ...
    for i, (agent, mr) in enumerate(zip(completion_data, makespan_ratios)):
        ...
        if ttg != float("inf"):
            ...
            print(
                f"Robot {agent.get('id', i)}: TTG = ∞, MR = N/A ✗ (did not reach goal)"
            )
...
# Standardized environment layouts and visualization parameters shared by all
...
class StandardizedEnvironment:
    ...
    def get_hallway_obstacles(cls):
        ...
        bottom_wall_xs = np.linspace(
            cls.GRID_X_MIN, cls.GRID_X_MAX, 12
    ...
    def get_intersection_obstacles(cls):
        ...
        for y in np.linspace(
            cls.GRID_Y_MIN, cls.GRID_Y_MAX, 12
        ):  # Reduced from 25 to 12
        ...
        for wall_pos in walls:
            ...
            if not (
                intersection_min <= x <= intersection_max
                and intersection_min <= y <= intersection_max
            ):
    ...
    def get_standard_agent_positions(cls, env_type, num_agents=2):
        ...
        if env_type == "doorway":
            ...
            positions = [
                {
                    "start": [-4.0, corridor_center - 0.3],
                    "goal": [4.0, corridor_center + 0.3],
                },  # Lower path
                {
                    "start": [4.0, corridor_center + 0.3],
                    "goal": [-4.0, corridor_center - 0.3],
            ...
            positions = [
                {
                    "start": [-4.0, corridor_center - 0.5],
                    "goal": [4.0, corridor_center + 0.5],
                },  # Offset horizontal
                {
                    "start": [corridor_center - 0.5, -4.0],
                    "goal": [corridor_center + 0.5, 4.0],
                },  # Offset vertical
            ]
            ...
            positions = [
                {"start": [0.0, 3.0], "goal": pad},  # Approach from top (through gap)
                {
                    "start": [0.0, -3.0],
                    "goal": pad,
                },  # Approach from bottom (through gap)
                {
                    "start": [-2.5, 0.0],
                    "goal": pad,
                },  # Approach from left (inner airspace)
                {
                    "start": [2.5, 0.0],
                    "goal": pad,
                },  # Approach from right (inner airspace)
            ]
        else:
            # Generic positions
            positions = [
    ...
    def create_standard_plot(cls, env_type, show_obstacles=True):
        ...
        if show_obstacles:
            ...
            if env_type == "landing_pad":
                pad_circle = patches.Circle(
                    cls.LANDING_PAD_CENTER,
                    radius=cls.LANDING_PAD_RADIUS,
                    facecolor="yellow",
                    edgecolor="red",
                    linewidth=2,
                    alpha=0.4,
                ...
                ax.plot(
                    cls.LANDING_PAD_CENTER[0],
                    cls.LANDING_PAD_CENTER[1],
                    "P",
                    color="red",
                    markersize=15,
                    zorder=2,
            ...
            for obs in obstacles:
                circle = patches.Circle(
                    obs,
                    radius=cls.DEFAULT_AGENT_RADIUS,
                    facecolor="gray",
                    edgecolor="black",
                    alpha=0.7,
                )
    ...
    def plot_agent_trajectory(
        cls, ax, positions, agent_id, show_trajectory=True, show_timestamps=True
    ):
        """
