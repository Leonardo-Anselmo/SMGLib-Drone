...
def parse_command(argv):
    """Parse track-oriented and legacy command styles.

    New style:
        experiment_cli.py yield_control configs/track_policy_*.json
        experiment_cli.py trajectory_planner baseline configs/track_trajectory_*.json
        experiment_cli.py trajectory_planner llm configs/track_trajectory_*.json
        experiment_cli.py trajectory_planner lookahead configs/track_trajectory_*.json
        experiment_cli.py trajectory_planner compare_all configs/track_trajectory_*.json

    Legacy style is still accepted:
        experiment_cli.py landing_pad configs/*.json
    """
    track_name = None
...
def save_gif_standardized(
    agent_list,
    r_min,
    filename=None,
    fps=5,
    num_moving_agents=None,
    scenario_type="impc",
    agent_summary="default",
    frame_log=None,
):
    ...
    fig, ax = StandardizedEnvironment.create_standard_plot(
        scenario_type, show_obstacles=True
    )
    ...
    # Per-drone home pads are drawn only when uav.home_pad differs
    ...
    legend_handles, legend_labels = StandardizedEnvironment.create_standard_legend(
        n_moving
    )
    ax.legend(
        legend_handles,
    ...
    def animate(frame):
        ...
        for k, i in enumerate(dynamic_indices):
            ...
            p = pos[i]
            tte = (
                a.tte_history[min(frame, len(a.tte_history) - 1)]
                if hasattr(a, "tte_history") and a.tte_history
                else a.time_to_expiry
            )
            cargo = _CARGO.get(
                getattr(a, "cargo_type", ""), getattr(a, "cargo_type", "")[:3]
            )
            acuity = _ACUITY.get(
                getattr(a, "patient_acuity", ""), getattr(a, "patient_acuity", "")[:3]
            )
            score = scores.get(i)
            if allowed == i:
...
def generate_animation_standardized(
    agent_list,
    r_min,
    filename=None,
    num_moving_agents=None,
...
def _generate_animation_standardized_unused(
    agent_list,
    r_min,
    filename=None,
    num_moving_agents=None,
    ...
    for step in range(len(agent_list[0].position)):
        ...
        ax.set_xlim(
            StandardizedEnvironment.GRID_X_MIN, StandardizedEnvironment.GRID_X_MAX
        )
        ax.set_ylim(
            StandardizedEnvironment.GRID_Y_MIN, StandardizedEnvironment.GRID_Y_MAX
        )
        ax.set_aspect("equal")
        ...
        for obs in obstacles:
            circle = Circle(
                obs,
                radius=StandardizedEnvironment.DEFAULT_AGENT_RADIUS,
                facecolor="gray",
                edgecolor="black",
                alpha=0.7,
            )
        ...
        if scenario_type == "landing_pad":
            ...
            pad_circle = Circle(
                pad,
                radius=StandardizedEnvironment.LANDING_PAD_RADIUS,
                facecolor="yellow",
                edgecolor="red",
                linewidth=2,
                alpha=0.4,
            ...
            # Per-drone home pads are drawn only when uav.home_pad
        ...
        n_moving = (
            num_moving_agents if num_moving_agents is not None else len(agent_list)
        )
        for i in range(n_moving):
            agent = agent_list[i]
            color = colors[i % len(colors)]
            ...
            if pos_index > 5:
                completed_positions = np.array(
                    agent.position[max(0, pos_index - 5) : pos_index + 1]
                )
                ax.plot(
...
def setup_standardized_scenario(env_type):
    ...
    print(
        f"Setting up {env_type.capitalize()} Environment using standardized configuration..."
    )
...
def main():
    ...
    obstacle_agents_x, obstacle_agents_v, obstacle_agents_target = (
        setup_standardized_scenario(env_type)
    )
    ...
    if scenario_config:
        ...
        policy_recipe = (
            scenario_config.get("policy") if env_type == "landing_pad" else None
        )
        use_planner = scenario_config.get("use_trajectory_planner", False)
        if track_name == "yield_control":
            if policy_recipe is None:
                raise ValueError(
                    "yield_control requires a config with a `policy` block."
                )
            use_planner = False
        ...
        if policy_recipe is not None and use_planner:
            raise ValueError(
                "Config must choose either `policy` or `use_trajectory_planner`, not both."
            )
        ...
        if use_planner and cargo_configs is not None:
            ...
            return_points = [
                np.array(d.get("return_point", d["start"]), dtype=float) for d in drones
            ]
            round_trip_params = {
        ...
        min_radius = get_input(
            "Enter minimum distance between drones",
            StandardizedEnvironment.DEFAULT_COLLISION_DISTANCE,
            float,
        )

        # Add configurable wall collision distance
        wall_collision_multiplier = get_input(
            "Enter wall collision distance multiplier (1.5-3.0 recommended)", 2.0, float
        )
        ...
        for i in range(num_moving_drones):
            ...
            defaults = (
                standard_positions[i]
                if i < len(standard_positions)
                else standard_positions[0]
            )

            # Get start position
            start_x = get_input(
                f"Start X position (default: {defaults['start'][0]})",
                defaults["start"][0],
                float,
            )
            start_y = get_input(
                f"Start Y position (default: {defaults['start'][1]})",
                defaults["start"][1],
                float,
            )
            ...
            # Get goal position
            goal_x = get_input(
                f"Goal X position (default: {defaults['goal'][0]})",
                defaults["goal"][0],
                float,
            )
            goal_y = get_input(
                f"Goal Y position (default: {defaults['goal'][1]})",
                defaults["goal"][1],
                float,
            )
        ...
        if env_type == "landing_pad":
            ...
            if use_priority.lower() == "y":
                ...
                for i in range(num_moving_drones):
                    dfl = (
                        default_cargos[i]
                        if i < len(default_cargos)
                        else default_cargos[-1]
                    )
                    print(f"\n  Drone {i} cargo:")
                    ct = get_input(f"    Cargo type", dfl["cargo_type"], str)
                    te = get_input(
                        f"    Time to expiry (steps)", dfl["time_to_expiry"], float
                    )
                    pa = get_input(f"    Patient acuity", dfl["patient_acuity"], str)
                    cargo_configs.append(
                        {"cargo_type": ct, "time_to_expiry": te, "patient_acuity": pa}
                    )
    ...
    print("\nStarting simulation...")
    result, agent_list, completion_step, frame_log = PLAN(
        num_drones,
        ini_x,
        ini_v,
        target,
    ...
    if result:
        ...
        generate_animation_standardized(
            agent_list,
            min_radius,
            filename=gif_filename,
            num_moving_agents=num_moving_drones,
            scenario_type=env_type,
            agent_summary=agent_summary,
            frame_log=frame_log,
        )
    else:
