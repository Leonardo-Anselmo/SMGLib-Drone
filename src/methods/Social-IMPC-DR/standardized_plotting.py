...
def plot_trajectory_standardized(agent_list, r_min, env_type="doorway"):
    ...
    fig, ax = StandardizedEnvironment.create_standard_plot(
        env_type, show_obstacles=True
    )

    # Plot each agent's trajectory
    for i, agent in enumerate(agent_list):
        # Determine if agent is stationary (obstacle)
        is_stationary = len(agent.position) <= 1 or all(
            np.allclose(agent.position[0], p) for p in agent.position
        )
        ...
        if is_stationary:
            ...
            color = StandardizedEnvironment.AGENT_COLORS[
                i % len(StandardizedEnvironment.AGENT_COLORS)
            ]
    ...
    num_moving_agents = sum(
        1
        for agent in agent_list
        if len(agent.position) > 1
        and not all(np.allclose(agent.position[0], p) for p in agent.position)
    )
    ...
    print(
        "Standardized trajectory plot saved as 'trajectory_standardized.svg' and 'trajectory_standardized.png'"
    )
...
def plot_environment_comparison():
    ...
    for i, env_type in enumerate(env_types):
        ...
        for j, pos in enumerate(positions):
            color = StandardizedEnvironment.AGENT_COLORS[
                j % len(StandardizedEnvironment.AGENT_COLORS)
            ]
            ...
            ax.plot(
                [pos["start"][0], pos["goal"][0]],
                [pos["start"][1], pos["goal"][1]],
                color=color,
                linestyle="--",
                alpha=0.5,
            )
    ...
    print(
        "Environment comparison plot saved as 'environment_comparison.png' and 'environment_comparison.svg'"
    )


def create_standardized_animation_frames(
    agent_list, env_type="doorway", num_moving_agents=None
):
    """
    ...
    for step in range(max_frames):
        ...
        for i, agent in enumerate(agent_list):
            ...
            is_moving = (num_moving_agents is None or i < num_moving_agents) and len(
                agent.position
            ) > 1
