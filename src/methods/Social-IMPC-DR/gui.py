...
class Model2(QObject):
    ...
    def solve(self, Num, ini_x, ini_v, target, r_min, epsilon, h, K, episodes):

        result, agent_list = PLAN(
            Num=Num,
            ini_x=ini_x,
            ini_v=ini_v,
            target=target,
    ...
    def save_trajectory(self):
        ...
        print("trajectory has been saved in 'trajectory.svg'")
...
class RunDrone2(QThread):
    ...
    def __init__(self, drones, model, fps):
        ...
        if not os.path.exists("path_deviation_csv"):
            os.mkdir("path_deviation_csv")
        self.path_data = {i: [] for i in range(self.drone_num)}

    @pyqtSlot()
    def run(self):
        ini_x = [self.model.start_point(j) for j in range(self.drone_num)]
        target = self.model.goal[: self.drone_num]
        nominal_positions = {j: list(ini_x[j]) for j in range(self.drone_num)}
        ...
        while True:
            ...
            for j in range(self.drone_num):
                ...
                # Compute nominal position ensuring it reaches the exact target
                num_steps = self.model.count if self.model.count > 0 else 1
                step_x = (target[j][0] - ini_x[j][0]) / num_steps
                step_y = (target[j][1] - ini_x[j][1]) / num_steps

                nominal_positions[j][0] += step_x
                nominal_positions[j][1] += step_y

                self.path_data[j].append(
                    [pos[0], pos[1], nominal_positions[j][0], nominal_positions[j][1]]
                )
                print("HERE")

            self.sig_drone.emit(drone_pos, drone_trace, drone_future, drone_terminal, i)
            i += 1
            if i >= self.model.count:
                break

        # Save data to CSV after simulation completes
        for j in range(self.drone_num):
            filename = f"path_deviation_robot_{j}.csv"
            with open(filename, mode="w", newline="") as file:
                writer = csv.writer(file)
                writer.writerow(["Actual X", "Actual Y", "Nominal X", "Nominal Y"])
                writer.writerows(self.path_data[j])
...
class DroneSimulatorPlot(FigureCanvas):
    ...
    def paint_drone2(self, drone_pos, drone_trace, drone_future, drone_terminal, i):
        ...
        for drone_id, (pos, trace_data, future_path, terminal_index) in enumerate(
            zip(drone_pos, drone_trace, drone_future, drone_terminal)
        ):
            ...
            self.__drone[drone_id] = Circle(
                pos,
                radius=self.radius,
                edgecolor="black",
                facecolor=self.COLORMAP[drone_id],
                zorder=4,
            )
            ...
            self.__future_path[drone_id] = Line2D(
                *zip(*future_path), linestyle=":", color=self.COLORMAP[drone_id], lw=2
            )

            self.axes.add_line(self.__future_path[drone_id])

            self.__traces[drone_id] = Line2D(
                *zip(*trace_data), linestyle="-", color=self.COLORMAP[drone_id], lw=2
            )
...
class ControlPanel(Panel):
    ...
    def __setup_ui(self):
        ...
        parameter_box.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Expanding)
    ...
    def set_random_ini(self):
        ...
        if Num:
            ...
            if times == 99:
                print(
                    "cannot find initial positions, please decrease the number of robots"
                )
        else:
    ...
    def set_random_target(self):
        ...
        if Num:
            ...
            if times == 99:
                print(
                    "cannot find target positions, please decrease the number of robots"
                )
        else:
    ...
    def __move_drone2(self, pos, trace_data, future_path, terminal_index, i):
        self.panel_animation.move_drone2(
            pos, trace_data, future_path, terminal_index, i
        )
    ...
    def __run2(self):
        ...
        self.__thread = RunDrone2(
            self.__drones, self.model2, fps=1 / self.h.value() * self.time.value()
        )
