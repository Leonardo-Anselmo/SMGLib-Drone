...
def save_video(
    frames, filename=None, fps=5, scenario_type="impc", agent_summary="default"
):
...
def save_gif(
    ...
    if num_moving_agents is None:
        ...
        def is_stationary(a):
            return len(a.position) > 1 and all(
                np.allclose(a.position[0], p) for p in a.position
            )

        dynamic_indices = [i for i, a in enumerate(agent_list) if not is_stationary(a)]
        obstacle_indices = [
            i for i in range(len(agent_list)) if i not in dynamic_indices
        ]
    else:
...
def main():
    ...
    if env_type == "doorway":
        obstacle_agents_x, obstacle_agents_v, obstacle_agents_target = (
            setup_doorway_scenario()
        )
    elif env_type == "hallway":
        obstacle_agents_x, obstacle_agents_v, obstacle_agents_target = (
            setup_hallway_scenario()
        )
    elif env_type == "intersection":
        obstacle_agents_x, obstacle_agents_v, obstacle_agents_target = (
            setup_intersection_scenario()
        )
    ...
    if env_type == "doorway":
        ...
        wall_collision_multiplier = get_input(
            "Enter wall collision distance multiplier (1.2-2.0 recommended)", 1.5, float
        )
        ...
        wall_collision_multiplier = get_input(
            "Enter wall collision distance multiplier (1.0-1.5 recommended)", 1.2, float
        )
        epsilon = get_input(
            "Enter epsilon value", 0.1, float
        )  # Increased for faster convergence
        step_size = get_input(
            "Enter step size", 0.1, float
        )  # Increased for faster movement
        k_value = get_input("Enter k value", 10, int)  # Reduced for faster computation
        max_steps = get_input(
            "Enter maximum number of steps", 100, int
        )  # Reduced for faster completion
    elif env_type == "intersection":
        # Intersection: Aggressive parameters for performance
        wall_collision_multiplier = get_input(
            "Enter wall collision distance multiplier (1.0-1.3 recommended)", 1.1, float
        )
        epsilon = get_input(
            "Enter epsilon value", 0.15, float
        )  # Increased for faster convergence
        step_size = get_input(
            "Enter step size", 0.15, float
        )  # Increased for faster movement
        k_value = get_input("Enter k value", 8, int)  # Reduced for faster computation
        max_steps = get_input(
            "Enter maximum number of steps", 80, int
        )  # Reduced for faster completion
    ...
    if result:
        ...
        print(
            "Using standardized environment configuration for consistent visualization."
        )
        generate_animation(
            agent_list,
            min_radius,
            ...
            agent_summary=str(num_moving_drones),
        )
    else:
